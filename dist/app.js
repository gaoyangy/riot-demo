...
class Request {
  constructor(opts) {
    this.opts = {
      ...
      ajax: savUtil_cjs.ajax
    };
    this.invokeQueues = [this.invoke.bind(this)];
    this.invoker = null;
    savUtil_cjs.bindEvent(this);
    if (opts) {
  ...
  request(options) {
    ...
    if (!this.invoker) {
      this.invoker = savUtil_cjs.compose(this.invokeQueues);
    }
    let { invoker } = this;
  ...
  invoke(ctx, next) {
    return new Promise((resolve, reject) => {
      ctx.xhr = savUtil_cjs.ajax(ctx.request, (err, data, headers) => {
        if (err) {
          return reject(err);
        }
        try {
...
function getRoutes(vueRoute, vueRouter, payloads, flux) {
  let routes = payloads.reduce((routes, payload) => {
    if (savUtil_cjs.isFunction(payload)) {
      payload = payload(vueRoute);
    }
    if (Array.isArray(payload) || savUtil_cjs.isObject(payload)) {
...
var savFlux_cjs_6 = savFlux_cjs$1.FluxRiot;
...
savFlux_cjs_6({flux, riot: riot_compiler});
